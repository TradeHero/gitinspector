...
__metric_cc_tokens__ = [[["java", "js", "c", "cc", "cpp"],
                         ["else", "for\s+\(.*\)", "if\s+\(.*\)", "case\s+\w+:", "default:", "while\s+\(.*\)"],
...
class Metrics(Outputable):
    def output_text(self):
        ...
        if metrics_logic.cyclomatic_complexity_density:
            ...
            for i in sorted(set([(j, i) for (i, j) in metrics_logic.cyclomatic_complexity_density.items()]),
                            reverse=True):
    ...
    def output_html(self):
        ...
        if metrics_logic.eloc:
            ...
            for n, i in enumerate(sorted(set([(j, i) for (i, j) in metrics_logic.eloc.items()]), reverse=True)):
                metrics_xml += "<div class=\"" + __get_metrics_score__(__metric_eloc__[FileDiff.get_extension(i[1])],
                                                                       i[0]) + \
                               (" odd\">" if n % 2 == 1 else "\">") + \
        ...
        if metrics_logic.cyclomatic_complexity:
            ...
            for n, i in enumerate(
                    sorted(set([(j, i) for (i, j) in metrics_logic.cyclomatic_complexity.items()]), reverse=True)):
        ...
        if metrics_logic.cyclomatic_complexity_density:
            ...
            for n, i in enumerate(
                    sorted(set([(j, i) for (i, j) in metrics_logic.cyclomatic_complexity_density.items()]),
                           reverse=True)):
                metrics_xml += "<div class=\"" + __get_metrics_score__(METRIC_CYCLOMATIC_COMPLEXITY_DENSITY_THRESHOLD,
                                                                       i[0]) + \
                               (" odd\">" if n % 2 == 1 else "\">") + \
